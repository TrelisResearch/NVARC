...
## Prolog Style: Checker (Validation Only)

Write simple checker-style Prolog that validates grids. Use standard Prolog operators:

| Operation | Syntax | Example |
|-----------|--------|---------|
| Equals | `A == B` or `A = B` | `Value == 0` |
| Not equals | `A \== B` | `Value \== 0` |
| Greater/less | `A >= B`, `A =< B`, `A > B`, `A < B` | `Rows >= 2` |
| Arithmetic | `X is A + B` | `Total is Rows * Cols` |
...
valid_input(Grid) :-
    ...
    maplist(valid_color, [A,B,C,D,E,F,G,H,I]),

    % Diagonal cells are equal
    A == E, E == I,

    % Diagonal value is non-zero
    A \== 0,
